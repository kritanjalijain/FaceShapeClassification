...
#load the image
imagepath = "testing_set\Oval\oval (51).jpg"
#haarcascade for detecting faces
# link = https://github.com/opencv/opencv/tree/master/data/haarcascades
face_cascade_path = "haarcascades\haarcascades\haarcascade_frontalface_default.xml"
...
cv2.waitKey(0)
filename = 'results/result_landmark6.png'
cv2.imwrite(filename,landmark)
...
for (x,y,w,h) in faces:
    ...
    temp = original.copy()
    #getting area of interest from image i.e., forehead (33% of face)
    forehead = temp[y:y+int(0.33*h), x:x+w]
...
for i in range(1):
  if similarity<10:
    if angle<170:
      ...
      print('maybe round shape.Jawlines are not that angular')
      break
  if line3<line1:
    if angle<160:
      print('heart shape.Forehead is more wider') 
      break
  if ovalsimilarity<10:
    print('oval shape. line2 & line4 are similar and line2 is slightly larger')
    break
  if line4 > line2:
    if angle<160:
...
cv2.waitKey(0)
filename1 = 'results/result_prediction6.png'
cv2.imwrite(filename1,output)